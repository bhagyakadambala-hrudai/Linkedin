...
// Model fallback chain — direct REST (no SDK headers that trigger IP rate-limiting)
const GEMINI_MODELS = ['gemini-2.0-flash-lite', 'gemini-1.5-flash-8b', 'gemini-2.0-flash', 'gemini-1.5-flash'];
...
async function generateContent(postType, role, skills, topic) {
  ...
  console.log(`[automation] GEMINI_API_KEY prefix: ${GEMINI_API_KEY.slice(0, 8)}...`);
