...
function parsePostContent(postType, raw) {
  ...
  if (postType === 2) {
    ...
    // Fall back to raw content if structured parsing yields empty text
  ...
  // Fall back to raw content if TEXT: label is missing
...
function layoutSteps(imageData) {
  ...
  sections.slice(0, 4).forEach((sec, i) => {
    ...
    let cy = COL_Y + 34;
    hLines.forEach((l, li) => { e += `<text x="${cx+28}" y="${cy + li*24}" font-family="${FONT}" font-size="18" font-weight="700" fill="${DARK}">${l}</text>`; });
    ...
    sec.bullets.slice(0, 4).forEach(b => {
      ...
      bLines.forEach((bl, li) => { e += `<text x="${cx+44}" y="${cy+li*20}" font-family="${FONT}" font-size="14.5" fill="${MUTED}">${bl}</text>`; });
...
function layoutSplit(imageData) {
  ...
  tLines.forEach((l, i) => { e += `<text x="${LEFT_W/2}" y="${startTY + i*(tSize+8)}" font-family="${FONT}" font-size="${tSize}" font-weight="700" fill="#ffffff" text-anchor="middle">${l}</text>`; });
  ...
  takL.forEach((l, i) => { e += `<text x="${LEFT_W/2}" y="${H-30+i*18}" font-family="${FONT}" font-size="11.5" fill="#94a3b8" text-anchor="middle" font-style="italic">${l}</text>`; });
  ...
  sections.slice(0, 4).forEach((sec, i) => {
    ...
    hLines.forEach((l, li) => { e += `<text x="${RX+36}" y="${cy+18+li*22}" font-family="${FONT}" font-size="17" font-weight="700" fill="#1a1a1a">${l}</text>`; });
...
function layoutGrid(imageData) {
  ...
  tLines.forEach((l, i) => { e += `<text x="${W/2}" y="${TITLE_Y + i*(tSize+6)}" font-family="${FONT}" font-size="${tSize}" font-weight="700" fill="${DARK}" text-anchor="middle">${l}</text>`; });
  ...
  sections.slice(0, 4).forEach((sec, i) => {
    ...
    hLines.forEach((l, li) => { e += `<text x="${px}" y="${cy+li*23}" font-family="${FONT}" font-size="18" font-weight="700" fill="${DARK}">${l}</text>`; });
    ...
    sec.bullets.slice(0, 3).forEach(b => {
      ...
      bLines.forEach((bl, li) => { e += `<text x="${px+18}" y="${cy+li*21}" font-family="${FONT}" font-size="15" fill="${TEXT}">${bl}</text>`; });
...
async function generateInfographicPNG(imageData) {
  ...
  const resvg = new Resvg(svg, {
    font: { loadSystemFonts: false, fontFiles, defaultFontFamily: 'Roboto' },
  });
...
async function uploadImageToLinkedIn(accessToken, personId, pngBuffer) {
  ...
  if (!regRes.ok) { const detail = await regRes.text().catch(() => ''); throw new Error(`LinkedIn registerUpload ${regRes.status}: ${detail}`); }
  ...
  if (!uploadUrl || !asset) throw new Error('LinkedIn registerUpload: missing uploadUrl or asset');
  const upRes = await fetch(uploadUrl, { method: 'PUT', headers: { 'Content-Type': 'application/octet-stream' }, body: pngBuffer });
  if (!upRes.ok && upRes.status !== 201) { const detail = await upRes.text().catch(() => ''); throw new Error(`LinkedIn image PUT ${upRes.status}: ${detail}`); }
  return asset;
}
...
// OpenRouter model fallback chain — verified working free-tier models
...
async function generateContent(postType, role, skills, topic) {
  ...
  console.log(`[automation] Using OPENROUTER_API_KEY prefix: ${OPENROUTER_API_KEY.slice(0, 12)}...`);
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'HTTP-Referer': 'https://linkedin-theta-seven.vercel.app',
          'X-Title': 'AutoLink AI',
        },
        body: JSON.stringify({ model: modelName, messages: [{ role: 'user', content: prompt }], max_tokens: 1200 }),
      ...
      if (!res.ok) {
        ...
        const errMsg = data?.error?.message || (typeof data?.error === 'string' ? data.error : '') || `HTTP ${status}`;
        console.error(`[automation] OpenRouter ${status} for ${modelName}: ${JSON.stringify(data)}`);
        if (status === 401 || status === 403) throw new Error(`[${status}] ${errMsg}`);
        lastErr = new Error(`[${status}] ${errMsg}`);
        continue;
      }

      const text = data?.choices?.[0]?.message?.content?.trim();
      if (!text) {
        console.error(`[automation] Empty response for ${modelName}: ${JSON.stringify(data)}`);
        lastErr = new Error('Empty response from OpenRouter');
        continue;
      }

      console.log(`[automation] SUCCESS with model: ${modelName}`);
      return text;
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      if (msg.includes('[401]') || msg.includes('[403]')) throw err;
      const m = msg.toLowerCase();
      const isRetryable = msg.includes('429') || m.includes('quota') || m.includes('rate limit') ||
        m.includes('too many') || m.includes('exhausted') || m.includes('overloaded') ||
        err.name === 'AbortError' || err.name === 'TimeoutError' || m.includes('timeout') || m.includes('abort');
      console.error(`[automation] Model ${modelName} failed (retryable=${isRetryable}): ${msg}`);
      lastErr = err;
...
async function getLinkedInPersonId(accessToken) {
  ...
  if (!res.ok) { const detail = await res.text().catch(() => ''); throw new Error(`LinkedIn userinfo ${res.status}: ${detail}`); }
...
async function publishToLinkedIn(accessToken, personId, text, imageAsset) {
  const shareContent = imageAsset
    ? { shareCommentary: { text }, shareMediaCategory: 'IMAGE', media: [{ status: 'READY', media: imageAsset }] }
  ...
  if (!res.ok) { const detail = await res.text().catch(() => ''); throw new Error(`LinkedIn ugcPosts ${res.status}: ${detail}`); }
  const data = await res.json();
...
async function runAutomation(userId) {
  ...
  const { data: profile, error: profileErr } = await admin
    .from('profiles').select('role, skills, topics, linkedin_token').eq('user_id', userId).maybeSingle();
  ...
  const { data: rotation } = await admin
    .from('automation_rotation').select('current_step, post_type').eq('user_id', userId).maybeSingle();
  ...
  if (!text || text.trim().length < 10) {
    console.error(`[automation] Parsed text too short (${text?.length ?? 0} chars) for postType=${postType}. Raw: ${rawContent?.slice(0, 200)}`);
    throw new Error(`AI returned unusable content for post type ${postType}. Raw output: ${rawContent?.slice(0, 100)}`);
  }
  ...
  const { postId, postUrl } = await publishToLinkedIn(profile.linkedin_token, personId, text, imageAsset);

  await admin.from('posts').insert({
    user_id: userId, content: text, status: 'published',
    post_id: postId, post_url: postUrl, posted_at: new Date().toISOString(), topic, post_type: postType,
  });
