...
// OpenRouter model fallback chain — all free-tier models
...
async function generateContent(postType, role, skills, topic) {
  ...
  console.log(`[automation] OPENROUTER_API_KEY prefix: ${OPENROUTER_API_KEY.slice(0, 8)}...`);
  ...
  for (const modelName of OPENROUTER_MODELS) {
    try {
      ...
      console.log(`[automation] Generated content with model: ${modelName}`);
      return text;
    } catch (err) {
