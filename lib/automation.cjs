...
// OpenRouter model fallback chain — stable free-tier models
  ...
  for (const modelName of OPENROUTER_MODELS) {
    try {
      ...
      if (!res.ok) {
        ...
        console.error(`[automation] OpenRouter ${res.status} for ${modelName}: ${JSON.stringify(data)}`);
      ...
      const isQuota = msg.includes('429') || m.includes('quota') || m.includes('rate') ||
        m.includes('too many') || m.includes('exhausted');
      console.error(`[automation] Model ${modelName} failed (quota=${isQuota}): ${msg}`);
      lastErr = err;
