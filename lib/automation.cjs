...
// OpenRouter model fallback chain — proven stable free-tier models
  ...
  for (const modelName of OPENROUTER_MODELS) {
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        signal: AbortSignal.timeout(25000), // 25s per model — prevent hanging
      });
...
async function runAutomation(userId) {
  ...
  // Parse into text caption + optional image data
  const { text, imageData } = parsePostContent(postType, rawContent);

  // Guard: don't attempt to publish an empty post
